...
object KNNTrain {
  def main(args: Array[String]): Unit ={
    ...
    val spark = SparkSession.builder().appName("KNNTrain").getOrCreate()
    ...
    /** *****=======5.通过决策树建立模型============= *******/
    val rfc: KNNClassifier = new KNNClassifier().setLabelCol("lable").setFeaturesCol("scaledFeatures")
    ...
    val paramMap: Array[ParamMap] = new ParamGridBuilder()
      ...
      .addGrid(rfc.bufferSize , Array(50.0, 100.0, 200.0,1000.0))
      .addGrid(rfc.topTreeSize, Array(df.count().toInt / 20))
      .build()
    ...
    /** *****=======13.模型保存============= *******/
    val rfcBestModel: KNNClassificationModel = bestModel.asInstanceOf[PipelineModel].stages(3).asInstanceOf[KNNClassificationModel]
