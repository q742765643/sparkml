...
object LogisticTrain {
  def main(args: Array[String]): Unit ={
    ...
    /** *****=======5.通过决策树建立模型============= *******/
    val rfc: LogisticRegression = new LogisticRegression().setLabelCol("lable").setFeaturesCol("features")
    ...
    /** *****=======13.模型保存============= *******/
    val rfcBestModel: LogisticRegressionModel = bestModel.asInstanceOf[PipelineModel].stages(2).asInstanceOf[LogisticRegressionModel]
    rfcBestModel.save("D:/spark.logicmodel")
    //加载模型
