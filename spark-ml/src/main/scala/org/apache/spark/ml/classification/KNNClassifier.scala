...
with KNNModelParams with HasWeightCol with MLWritable with Serializable {
 /* require(subTrees.getStorageLevel != StorageLevel.NONE,
  ...
  override def write: MLWriter = new KNNClassificationModel.KNNClassificationModelWriter(this)
...
object KNNClassificationModel extends MLReadable[KNNClassificationModel] {
  ...
  @Since("2.0.0")
  override def read: MLReader[KNNClassificationModel] =
    new KNNClassificationModelReader

  @Since("2.0.0")
  override def load(path: String): KNNClassificationModel = super.load(path)

  private[KNNClassificationModel]
  class KNNClassificationModelWriter(instance: KNNClassificationModel)
    extends MLWriter {

    override protected def saveImpl(path: String): Unit = {
      val extraMetadata: JObject = Map(
        "numClasses" -> instance._numClasses)
      DefaultParamsWriter.saveMetadata(instance, path, sc, Some(extraMetadata))
      val dataPath = new Path(path, "data").toString
      instance.subTrees.saveAsObjectFile(dataPath)
      val top = new Path(path, "top").toString
      var topTree:RDD[Tree]=sparkSession.sparkContext.makeRDD(Seq(instance.topTree.value))
      topTree.saveAsObjectFile(top)
    }

  }

  private class KNNClassificationModelReader
    extends MLReader[KNNClassificationModel] {
    ...
    override def load(path: String): KNNClassificationModel = {
      ...
      val dataPath = new Path(path, "data").toString
      val topPath = new Path(path, "top").toString
      val subTrees:RDD[Tree] = (sparkSession.sparkContext.objectFile(dataPath)).repartition(1);
      val topTree:RDD[Tree] = (sparkSession.sparkContext.objectFile((topPath))).repartition(1);
      var tree:Tree=topTree.collect().apply(0)
      val model = new KNNClassificationModel(metadata.uid,sparkSession.sparkContext.broadcast(tree), subTrees, numClasses)
